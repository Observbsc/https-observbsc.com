...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: observBalance, fetchStatus: observFetchStatus } = useTokenBalance(tokens.observ.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('OBSERV Balance')}</Text>
        {observFetchStatus !== FetchStatus.Fetched ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(observBalance, 9, 3)}</Text>
        )}
      </Flex>
